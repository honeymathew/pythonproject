    ...
    while True:
        ...
        if choice == 1:
            ...
            for line in lines:
                ...
                if file_id == id:
                    input("Alredy exist,press entr to co")
                    exists =True
                    break
            if exists:
                continue
            ...
            if search_choice == 1:
                ...
                for line in lines:
                    if line.strip().split("|")[2].strip() in (last_name.strip(),last_name.strip().capitalize()):
                       search_records.append(line.strip().split("|"))
            ...
            input_recod = [student_id.strip(), first_name.strip(), last_name.strip(), major.strip(), gpa.strip(), phone.strip(), dob.strip()]
